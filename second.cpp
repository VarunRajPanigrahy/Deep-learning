...
ll dfs1(ll s,ll p,ll par[],std::vector<ll> v[])
{  par[s]=p;
	for(auto x: v[s]){
		if(x==p) continue;
		//par[x]=s;
	...
	// debug(v[4].size());
     ...
     while(!q.empty()){
     	...
     	for(ll i=0;i<ss;i++){
     		...
     		if(visited[par[it.first]]){
     			visited[par[it.first]]=1;
     			//debug(par[it.first]);
     			
     			q.push({par[it.first],it.second+1});
     		
     		  
     		}
     		if(dp[par[it.first]]<1+dp[it.first]) dp[par[it.first]]=1+dp[it.first];
